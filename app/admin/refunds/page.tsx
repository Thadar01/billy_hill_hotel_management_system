    ...
    <Layout>
      <div className="p-6 text-black">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1 text-sm">Review and manage refund requests for the Finance Team</p>
        </div>
            ...
            <div className="flex gap-2">
              {["all", "pending", "accepted", "rejected"].map((s) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm transition-colors ${statusFilter === s
                      ? "bg-black text-white"
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="bg-white rounded-2xl shadow overflow-x-auto border border-gray-100">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((refund) => (
                  <tr key={refund.refundID} className="border-b hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 capitalize">{refund.refundMethod}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${statusBadgeClass(refund.refundStatus)}`}>
                        {refund.refundStatus}
                      </span>
                    </td>
                        ...
                        {refund.refundStatus === "pending" && (
                          <>
                            <button
                              onClick={(e) => {
                                e.preventDefault();
                                handleUpdateStatus(refund.refundID, "accepted");
                              }}
                            ...
                            <button
                              onClick={(e) => {
                                e.preventDefault();
                                handleUpdateStatus(refund.refundID, "rejected");
                              }}
                              disabled={updatingRefundID === refund.refundID}
